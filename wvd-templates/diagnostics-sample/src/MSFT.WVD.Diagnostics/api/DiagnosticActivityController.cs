...
namespace MSFT.WVD.Diagnostics.api
{
    ...
    public class DiagnosticActivityController : ControllerBase
    {
        ...
        {
            _logger.LogInformation($"Make api call to get connection activities of user {upn.Replace(Environment.NewLine, "")} within tenant {tenant.Replace(Environment.NewLine, "")} within tenant group {tenantGroupName.Replace(Environment.NewLine, "")}");
        ...
        {
            _logger.LogInformation($"Make api call to get management activities of user {upn.Replace(Environment.NewLine, "")} within tenant {tenant.Replace(Environment.NewLine, "")} within tenant group {tenantGroupName.Replace(Environment.NewLine, "")}");
            string token = Request.Headers["Authorization"];
        ...
        {
            _logger.LogInformation($"Make api call to get feed activities of user {upn.Replace(Environment.NewLine, "")} within tenant {tenant.Replace(Environment.NewLine, "")} within tenant group {tenantGroupName.Replace(Environment.NewLine, "")}");
            string token = Request.Headers["Authorization"];
